...
#Displays solution for current question
def makeSolution():
    qsolution.value="<font size=\"+1\">" + questionlist[curquestion][2]

#Hides solution for current question
def hideSolution():
...
#Next question then call display method
def nextq():
    global curquestion
...
#Logic to display part two of question if answer is false and has yet to be displayed
def launchifneeded():
...
#Checks current answer and increases numCorrect if correct
...
def clearbtn(btn):
    ...
    btn.disabled = False
#reset yes and no btn
def clearbtns():
    clearbtn(btn_yes)
    clearbtn(btn_no)
#Disable yes and no btn
def disablebtns():
    btn_yes.disabled=True
    btn_no.disabled=True
#Answer pressed clear and enable next btn
def answerpress(btn):
    clearbtns()
