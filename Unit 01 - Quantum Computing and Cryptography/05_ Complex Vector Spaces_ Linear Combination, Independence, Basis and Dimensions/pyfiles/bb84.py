...
#arr = os.listdir()
#print(arr)
#Creating arrays to hold inputs
rone = []
ronehelp = []
rtwo = []
rtwohelp = []
...
#changes the button to the opposite value when clicked and moves pictures
def swapChoice(btn):
...
#changes the button to the opposite value when clicked
def swapChoice2(btn):
    if(btn.description=="1"):
        btn.description="?"
    elif(btn.description=="0"):
...
#sets arrows to correct polarization
def checkpolar():
    for i in range(8):
...
#sets measurement arrows to correct value
def takemesurment():
    for i in range(8):
...
#takes in an index and returns the correct value
def checkKey(i):
...
#Randomise all
def randomAll(btn):
    reset("any input here its not used")
    resetbtn()
...
#sets the correct images in a row
def imagerow(row):
    clearcolor()
...
#Checks all on click and preformes based on correct or Incorrect answers
def checkall(btn):
    count = 0
    for i in range(8):
        if checkKey(i) == True:
